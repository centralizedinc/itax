    ...
    <a-table bordered :dataSource="data_source" :columns="columns" :pagination="false">
      ...
      <template slot="atc" slot-scope="text, record, index">
        <a-select style="width 100%" @change="e => pickAtc(e, index)" :defaultValue="text || null">
          ...
          <a-select-option
            :title="item.nature_payment"
            style="width:100%"
            v-for="item in atc_list"
            :key="item"
            width="100%"
          >{{item.atc}}</a-select-option>
        </a-select>
      </template>
      <template slot="nature_payment" slot-scope="text, record, index">
        <span>{{record.nature_payment}}</span>
      </template>
...
export default {
  ...
  methods: {
    ...
    pickAtc(val, index) {
      if (index > -1) {
        console.log("pick atc val: " + JSON.stringify(val));
        this.data_source[index].atc = val.atc;
        this.data_source[index].nature_payment = val.nature_payment;
    ...
    computeAtc(val, index) {
      ...
      if (index > -1) {
        ...
        this.data_source.forEach(data => {
          console.log("data amount: " + JSON.stringify(data.amount));
