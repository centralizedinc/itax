...
passport.use('login', new LocalStrategy({
    ...
    AccountDao.findByEmail(email)
        .then((account_result) => {
            if (!account_result) return done({ message: constant_helper.invalid_auth }, false);
            else if (account_result.status === 0) return done({ message: constant_helper.confirmation_required }, false);
            // Validate Password
            else {
        ...
        .then((isValid) => {
            if (!isValid) return done({ message: constant_helper.invalid_auth }, false);
            else {
...
passport.use('signup', new LocalStrategy({
    ...
    if (!email) return done({ message: constant_helper.invalid_email });
